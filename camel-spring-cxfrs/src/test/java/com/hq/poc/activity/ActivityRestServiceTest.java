...
public class ActivityRestServiceTest extends CamelSpringTestSupport {
	...
	public void setup() throws Exception {
		...
		MockActivityService activityService =
			context().getRegistry().lookupByNameAndType("activityService", MockActivityService.class);
	...
	public void testCreateActivity() throws Exception {
		...
		log.info(String.format("Created new activity with response [%s]", response));
		...
		response = template.requestBodyAndHeaders(
				...
				json, headers, String.class);
		log.info(String.format("Deleted new activity with response [%s]", response));
	}
